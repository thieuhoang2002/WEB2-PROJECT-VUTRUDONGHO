<div id="main">
    ...
    $url = isset($_GET['color']) && ($_GET['color'] > 0) ? $_GET['color'] : '';
    $total = $conn->prepare("select * from product where Status = 1 and Color = ?");
    $total->bind_param("s", $url);
    $total->execute();
    $total_a = $total->get_result();
    $total_a = $total_a->num_rows;
    $total_page = ceil($total_a / $item_page);
    $stmt = $conn->prepare("select * from product where Status=1 and Color = ? order by ProductID asc LIMIT " . $item_page . " OFFSET " . $offset);
    ...
    <div class="maincontent">
        ...
        <div class="pagination">
            ...
            if ($cur_page > 2) {
                $first_page = 1;
                ?>
                <a class="page-item" href="?page=<?= $first_page ?>&color=<?= $url ?>">First</a>
                <?php
            }
            if ($cur_page > 1) {
                $prev_page = $cur_page - 1;
                ?>
                <a class="page-item" href="?page=<?= $prev_page ?>&color=<?= $url ?>">Prev</a>
            <?php }
            ?>
            ...
            <?php for ($num = 1; $num <= $total_page; $num++) { ?>
                <?php if ($num != $cur_page) { ?>
                    <?php if ($num > $cur_page - 2 && $num < $cur_page + 2) { ?>
                        <a class="page-item" href="?page=<?= $num ?>&color=<?= $url ?>"><?= $num ?></a>
                    <?php } ?>
                <?php } else { ?>
            ...
            if ($cur_page < $total_page - 1) {
                ...
                <a class="page-item" href="?page=<?= $next_page ?>&color=<?= $url ?>">Next</a>
            <?php }
            if ($cur_page < $total_page - 2) {
                $end_page = $total_page;
                ?>
                <a class="page-item" href="?page=<?= $end_page ?>&color=<?= $url ?>">Last</a>
            <?php }
            ?>
        </div>
    </div>
