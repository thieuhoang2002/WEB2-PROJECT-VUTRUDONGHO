<div id="main">
    ...
    $stmt = $conn->prepare("select * from product where Status=1 and BrandID = ? order by ProductID asc LIMIT " . $item_page . " OFFSET " . $offset);
    ...
    <div class="maincontent">
        ...
        <div class="pagination">
            ...
            if ($cur_page > 2) {
                $first_page = 1;
                ?>
                <a class="page-item" href="?page=<?= $first_page ?>&idBrand=<?= $url ?>">First</a>
                <?php
            }
            if ($cur_page > 1) {
                $prev_page = $cur_page - 1;
                ?>
                <a class="page-item" href="?page=<?= $prev_page ?>&idBrand=<?= $url ?>">Prev</a>
            <?php }
            ?>
            ...
            <?php for ($num = 1; $num <= $total_page; $num++) { ?>
                <?php if ($num != $cur_page) { ?>
                    <?php if ($num > $cur_page - 2 && $num < $cur_page + 2) { ?>
                        <a class="page-item" href="?page=<?= $num ?>&idBrand=<?= $url ?>"><?= $num ?></a>
                    <?php } ?>
                <?php } else { ?>
            ...
            if ($cur_page < $total_page - 1) {
                ...
                <a class="page-item" href="?page=<?= $next_page ?>&idBrand=<?= $url ?>">Next</a>
            <?php }
            if ($cur_page < $total_page - 2) {
                $end_page = $total_page;
                ?>
                <a class="page-item" href="?page=<?= $end_page ?>&idBrand=<?= $url ?>">Last</a>
            <?php }
            ?>
        </div>
    </div>
